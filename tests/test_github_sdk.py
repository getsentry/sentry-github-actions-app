...
@patch("src.github_sdk.get_uuid")
def test_send_trace_retries_transient_sentry_ssl_errors(
    mock_get_uuid,
    mock_sleep,
    jobA_job,
    jobA_runs,
    ...
    envelope_url = "https://foo@random.ingest.sentry.io/api/bar/envelope/"
    responses.post(envelope_url, body=requests.exceptions.SSLError())
    responses.post(envelope_url, body=requests.exceptions.SSLError())
    responses.post(envelope_url)

    client = GithubClient(dsn=DSN, token=TOKEN)
    resp = client.send_trace(jobA_job)

    assert resp.status_code == 200
    assert [call.request.url for call in responses.calls].count(envelope_url) == 3

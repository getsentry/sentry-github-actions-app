...
@responses.activate
def test_send_trace_ignores_sentry_ssl_failure(
    caplog,
    jobA_job,
    jobA_runs,
    jobA_workflow,
    ...
    responses.post(
        "https://foo@random.ingest.sentry.io/api/bar/envelope/",
        body=requests.exceptions.SSLError("SSL connection closed unexpectedly"),
    )
    ...
    assert "Failed to send Sentry envelope" in caplog.text

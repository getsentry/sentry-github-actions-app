...
@responses.activate
def test_send_trace_ignores_connection_errors(
    jobA_job,
    jobA_runs,
    jobA_workflow,
):
...
@responses.activate
def test_send_trace_ignores_sentry_http_errors(
    jobA_job,
    jobA_runs,
    jobA_workflow,
):
    responses.get(jobA_job["run_url"], json=jobA_runs)
    responses.get(jobA_runs["workflow_url"], json=jobA_workflow)
    responses.post("https://foo@random.ingest.sentry.io/api/bar/envelope/", status=500)

...
def test_trace_generation_falls_back_on_run_timeout(jobA_job):
    client = GithubClient(dsn=DSN, token=TOKEN)
    with patch.object(client, "_fetch_github", side_effect=requests.ConnectTimeout):
        trace = client._generate_trace(jobA_job)

    assert trace["user"] == {}
    assert trace["contexts"]["trace"]["data"] == {"job": jobA_job["html_url"]}
    assert trace["tags"]["job_status"] == jobA_job["conclusion"]
    assert trace["tags"]["commit"] == jobA_job["head_sha"]
    assert trace["tags"]["run_attempt"] == jobA_job["run_attempt"]
    assert trace["tags"]["repo"] == "getsentry/sentry"
    assert "workflow" not in trace["tags"]
    assert "event" not in trace["tags"]
    assert "branch" not in trace["tags"]


def test_trace_generation_continues_when_workflow_fetch_times_out(jobA_job, jobA_runs):
    client = GithubClient(dsn=DSN, token=TOKEN)
    runs_response = Mock()
    runs_response.json.return_value = jobA_runs
    with patch.object(
        client,
        "_fetch_github",
        side_effect=[runs_response, requests.ConnectTimeout],
    ):
        trace = client._generate_trace(jobA_job)
    ...
    assert trace["tags"]["run_attempt"] == jobA_runs["run_attempt"]
    assert "workflow" not in trace["tags"]

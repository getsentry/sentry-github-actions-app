...
from src.github_sdk import ENVELOPE_UPLOAD_MAX_ATTEMPTS
from src.github_sdk import ENVELOPE_UPLOAD_TIMEOUT_SECONDS
...
@responses.activate
@patch("src.github_sdk.requests.post")
def test_send_trace_retries_transient_upload_errors(
    mock_post,
    jobA_job,
    jobA_runs,
    jobA_workflow,
    ...
    mock_post.side_effect = [
        requests.exceptions.SSLError("transient handshake failure"),
        success_response,
    ]

    client = GithubClient(dsn=DSN, token=TOKEN)
    response = client.send_trace(jobA_job)

    assert response is success_response
    assert mock_post.call_count == 2
    for call in mock_post.call_args_list:
        assert call.kwargs["timeout"] == ENVELOPE_UPLOAD_TIMEOUT_SECONDS


@responses.activate
@patch("src.github_sdk.requests.post")
def test_send_trace_returns_none_when_retries_exhausted(
    mock_post,
    caplog,
    jobA_job,
    jobA_runs,
    ...
    mock_post.side_effect = requests.exceptions.SSLError("still failing")
    client = GithubClient(dsn=DSN, token=TOKEN)
    ...
    assert mock_post.call_count == ENVELOPE_UPLOAD_MAX_ATTEMPTS
    assert "Failed sending Sentry envelope after" in caplog.text
